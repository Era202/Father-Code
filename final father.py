...
st.title("🚀 أداة تحليل BOM معالجة للأخطاء")
st.markdown("---")

# ==============================================================================
# 🔹 0. تهيئة Session State للحفاظ على النتائج
# ==============================================================================
...
# ==============================================================================
# 🔹 1. الشريط الجانبي للإعدادات (Sidebar for Configuration)
# ==============================================================================
...
uploaded_file = st.sidebar.file_uploader(
    "⬆️ ارفع ملف Excel",
    type=["xlsx"],
    help="يجب أن يحتوي الملف على شيت BOM. يفضل وجود شيتات 'father code' و 'MRP Control'."
)

if uploaded_file is None:
    st.info("👋 مرحبًا! يرجى رفع ملف Excel من الشريط الجانبي لبدء التحليل.")
    st.stop()

try:
    # --- قراءة الشيتات وتحديدها من الشريط الجانبي ---
    ...
    father_sheet = st.sidebar.selectbox("اختر شيت الـ Father (Parent-Child)", options=father_options, index=default_father)
    ...
    # --- قراءة البيانات بناءً على الاختيار ---
    ...
    # --- تحديد الأعمدة (بشكل تفاعلي إذا لزم الأمر) ---
    st.sidebar.markdown("---")
    st.sidebar.subheader("🔗 3. تحديد الأعمدة الرئيسية")
    ...
    # --- فلترة البيانات ---
    st.sidebar.markdown("---")
    st.sidebar.subheader("🔎 4. فلترة التحليل")

    parents_available = sorted(father_df[parent_col].dropna().unique().astype(str)) if father_df is not None and parent_col in father_df.columns else []
    ...
    if st.sidebar.button("🚀 تشغيل التحليل", type="primary"):
        with st.spinner("⏳ جاري معالجة البيانات... يرجى الانتظار..."):
            # تحويل الأعمدة الرئيسية إلى نص لضمان المطابقة
            ...
            mrp_dict = {}
            if mrp_control_df is not None:
                # إزالة التكرارات قبل إنشاء القاموس
                mrp_df_unique = mrp_control_df.drop_duplicates(subset=[mrp_component_col], keep='first')
                mrp_dict = mrp_df_unique.set_index(mrp_component_col).to_dict(orient='index')
            # **********************************
            ...
            with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
                ...
                for parent in selected_parents:
                    ...
                    for comp in parent_components:
                        ...
                        count = sum(1 for child in children if comp in bom_grouped.get(child, set()))
                        usage_pct = round(count / total_children * 100, 2) if total_children > 0 else 0.0
                        
                        usage_rows.append({
                            component_col: comp, "Num_Children_with_Component": count, "Total_Children": total_children,
                            "Usage_%": usage_pct, "MRP_Controller": mrp_info.get(mrp_controller_col), "Order_Type": mrp_info.get(mrp_order_type_col)
                        })
                    ...
                    summary_list.append({
                        "Parent_Code": parent, "Num_Children": total_children, "Total_Components": total_comps,
                        "Shared_Components": int(shared_comps), "Shared_Components_%": similarity_pct
                    })
    ...
    # ==============================================================================
    # 🔹 3. عرض النتائج (يتم العرض فقط إذا تم التحليل)
    # ==============================================================================
    if not st.session_state.analysis_complete:
        st.info("ℹ️ الإعدادات جاهزة! اضغط على زر 'تشغيل التحليل' في الشريط الجانبي لعرض النتائج.")
    else:
        ...
        with tab1:
            ...
            if not st.session_state.all_merged_df.empty:
                ...
                st.dataframe(
                    low_shared_df[['Parent', component_col, 'Num_Children_with_Component', 
                                   'Total_Children', 'Usage_%', 'MRP_Controller', 'Order_Type']].head(200)
                )
        with tab2:
            st.subheader("أعلى 10 مكونات انحرافًا على المستوى الإجمالي")
            st.dataframe(st.session_state.top10_global)
        with tab3:
            st.subheader("استعراض تفاصيل الانحراف لكل Parent على حدة")
